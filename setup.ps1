...
# 1. Kontrola / Instalace Chocolatey
...
# 3. Stažení modelu Whisper medium
$modelPath = ".\backend\whisper\models\ggml-medium.bin"
if (-not (Test-Path $modelPath)) {
    Write-Host "🧠 Stahuji AI model Whisper medium (~1.5 GB)..." -ForegroundColor Yellow
    New-Item -ItemType Directory -Force -Path ".\backend\whisper\models" | Out-Null
...
# 4. Kompilace aplikace
Write-Host "⚙️ Kompiluji Go backend..." -ForegroundColor Yellow
go mod tidy
go build -o backend/server.exe ./backend/main.go

Write-Host "🎉 Instalace dokončena! Aplikaci spustíte pomocí START.bat." -ForegroundColor Green